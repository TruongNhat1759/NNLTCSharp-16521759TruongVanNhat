...
namespace FrostPlay
{
    ...
    public sealed partial class MainWindow
    {
        ...
        //https://www.youtube.com/watch?v=G_YHM_-tUiY
        private void btnSearch_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var source = @"E:\Downloads\Mp3\";
                var youtube = YouTube.Default;
                var vid = youtube.GetVideo(urlDownload.Text);
            ...
            {
                urlDownload.Text = String.Empty;
            }
            urlDownload.Text = String.Empty;
        }
        private void timerBtn_Show(object sender, RoutedEventArgs e)
        {
        ...
        {
            List<Music> clonePlayList = new List<Music>(playList);
            ...
            if (!String.IsNullOrEmpty(searchBox.Text))
            {
                foreach (Music music in playList)
                {
                    ...
                    else if (music.title.StartsWith(searchBox.Text, StringComparison.CurrentCultureIgnoreCase))
                    {
                        playListBox.Items.Add(music);
                        refreshMusicMember(music, indexResultList);
                        indexResultList = indexResultList + 1;
                    }
                }
            } 
            else
            {
                for (int i = 0; i < clonePlayList.Count; i++)
                {
                    playListBox.Items.Add(clonePlayList[i]);
                    refreshMusicMember(clonePlayList[i], i);
                }
            }
        }
